...
def crop_and_recognize(image_path, results):
    ...
    final_text = ""  # Store the final text
    ...
    for idx, (bbox, score) in reversed(list(enumerate(zip(results[0], results[1])))):       
        ...
        # Get bounding rectangle (min/max x, y)
        ...
        # Convert to PIL format (required for TrOCR)
        ...
        # Append text to final result
        final_text += text + " "

        # Save cropped image (optional for debugging)
